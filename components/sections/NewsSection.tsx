...
export default function NewsSection({ posts }: NewsSectionProps) {
  ...
  const sectionCopy = ui.newsSection;
      ...
      <AnimatedSection animation="fade-up">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-12">
          <div>
            ...
            <h2 className="mt-2 text-3xl sm:text-4xl font-bold text-[#1f2a1f] dark:text-white break-words">
              {sectionCopy.title}
            </h2>
          </div>
          <a
            className="inline-flex items-center justify-center px-6 py-3 bg-[#1f2a1f] dark:bg-white text-[#ffffff] dark:text-zinc-900 rounded-xl font-medium text-center hover:opacity-90 transition no-underline shrink-0"
            href="/blog-left-sidebar"
          >
            {sectionCopy.viewAllLabel}
          </a>
        </div>
      </AnimatedSection>
        ...
        <AnimatedSection animation="fade-right" delay={100}>
          <article className="lg:col-span-3 relative group overflow-hidden rounded-2xl aspect-[4/3]">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent flex flex-col justify-end p-6 sm:p-8">
              ...
              <h3 className="mt-2 text-xl sm:text-2xl font-bold text-white break-words">
                {featured.title}
              </h3>
              ...
              <a
                href={featured.href}
                className="mt-4 inline-flex items-center justify-center px-6 py-3 bg-white/95 text-[#1f2a1f] rounded-xl font-medium text-center hover:bg-white transition no-underline self-start"
              >
                {sectionCopy.readArticleLabel}
              </a>
            </div>
          </article>
        </AnimatedSection>
