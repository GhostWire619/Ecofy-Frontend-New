    ...
    <section className="py-20 lg:py-28 max-w-7xl mx-auto px-4 sm:px-6">
      ...
      <AnimatedSection animation="stagger" stagger className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat) => (
          <article
            key={stat.label}
            className="bg-[#ffffff] dark:bg-zinc-900/80 border border-[#1f2a1f]/10 dark:border-zinc-800/50 rounded-2xl shadow-lg p-8 text-center transition-transform hover:-translate-y-1"
          >
            <strong className="block text-4xl font-bold text-[#1f2a1f] dark:text-white">
              {stat.value}
            </strong>
            <span className="mt-2 block text-sm text-[#5f695d] dark:text-zinc-400">
              {stat.label}
            </span>
          </article>
        ))}
